...
function UnreadBadge({ count }: { count: number }) {
  ...
  return (
    <span
      ...
      aria-label={`${count} support ticket${count === 1 ? "" : "s"} need attention`}
    >
      {count > 99 ? "99+" : count}
    </span>
  );
...
export default function SupportNavLink({
  ...
  const onTickets =
    pathname === "/support/tickets" || pathname.startsWith("/support/tickets/");
  const onAdmin = pathname === "/admin/support" || pathname.startsWith("/admin/support/");
  const isActive = onTickets || onAdmin;
  ...
  return (
    <Link
      href="/support"
      className={`flex w-full items-center gap-2 rounded-lg px-3 py-1.5 font-body text-sm transition ${
        onMember
